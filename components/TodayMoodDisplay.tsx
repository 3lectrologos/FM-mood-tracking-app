...
export default function TodayMoodDisplay({ data }: { data: DataPoint }) {
  return (
    <div className="flex w-full flex-col gap-250 desktop:flex-row desktop:gap-400">
      <MoodCard
        className="tablet:h-[340px] desktop:w-[670px]"
        mood={data.mood}
      />
      <div className="flex h-[340px] flex-col gap-y-250">
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center gap-200 tablet:w-[246px] tablet:items-start tablet:gap-150">
        ...
        <span className="text-center txt-preset-6-italic text-pretty tablet:text-start">
          {moodText}
        </span>
      </div>
    </DisplayCard>
